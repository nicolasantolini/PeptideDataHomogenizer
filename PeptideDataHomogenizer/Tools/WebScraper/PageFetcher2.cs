...
public class PageFetcher : IPageFetcher, IDisposable
{
    ...
    public PageFetcher([FromServices] IElsevierArticleFetcher elsevierArticleFetcher, [FromServices] ArticleExtractorFromHtml articleExtractor, [FromServices] HttpClient http, [FromServices] WileyArticleDownloader wileyArticleDownloader, [FromServices] PDBContextDataExtractor pDBContextDataExtractor, [FromServices] DatabaseDataHandler databaseDataHandler)
    {
    ...
    public async Task<List<Entities.Chapter>> GetFullPageContentAsync(string url, CancellationToken ct = default, string title = "")
    {
        ...
        try
        {
            ...
            catch (TimeoutException)
            {
                ...
                return new List<Entities.Chapter>(); // Return empty list on timeout
            }
            ...
            var discreditedPublishers = await DatabaseDataHandler.GetAllAsync<DiscreditedPublisher>();
            ...
            {
                chapters = new List<Entities.Chapter>
                {
            ...
            else
            {
                ...
                chapters = _articleExtractor.ExtractChapters(content, title,finalUrl);
